...
for (v = 0; v < u; v++)//This for loop is used to count the average throughput of changing the state matrix from all 0s to a state matrix that meets the requirements many times, such as 1000 times, 10000 times.
{
	...
	for (i = 0; i < m; i++) //Convert the coding matrix according to the state matrix.
	{
		for (j = 0; j < n; j++)
		{
			if (s[i][j] == 0)
			{
				a[i][j] = 1; //All the rows with 0 in the state matrix are selected as the lowest 0, and the corresponding position of the coding matrix becomes 1.
				break;
			}	    
		}
    }
